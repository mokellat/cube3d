...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/02/06 14:52:18 by mokellat          #+#    #+#             */
/*   Updated: 2021/02/07 18:37:23 by mokellat         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "cub3d.h"

void	rays_directions(void)
{
	normalise_angle();
	g_new_player.ray_down = (g_new_player.ray_angle > 0 &&
	g_new_player.ray_angle < M_PI);
	g_new_player.ray_up = !(g_new_player.ray_down);
	g_new_player.ray_right = (g_new_player.ray_angle < 0.5 * M_PI
	|| g_new_player.ray_angle > 1.5 * M_PI);
	g_new_player.ray_left = !g_new_player.ray_right;
}

void	help_inter_horz(void)
{
	while (g_nexthorztouchy >= 0 && g_nexthorztouchy <= g_tile * g_count &&
	g_nexthorztouchx >= 0 && g_nexthorztouchx <= g_tile * g_b_lenght)
	{
		if (haswallat(g_nexthorztouchx, g_nexthorztouchy - (
			g_new_player.ray_up ? 1 : 0)))
		{
			g_foundawallhorz = 1;
...
void	help_inter_ver(void)
{
	while (g_nextvertouchy >= 0 && g_nextvertouchy <= g_tile * g_count
	&& g_nextvertouchx >= 0 && g_nextvertouchx <= g_tile * g_b_lenght)
	{
		if (haswallat(g_nextvertouchx - (g_new_player.ray_left ? 1 : 0),
		g_nextvertouchy))
		{
			g_foundawallver = 1;
...
void	horz_intersection_calcul(void)
{
	rays_directions();
	normalise_angle();
	g_foundawallhorz = 0;
	...
	g_xinterupthorz = g_new_player.pos_x +
	((g_yinterupthorz - g_new_player.pos_y) / tan(g_new_player.ray_angle));
	g_ystephorz = g_tile;
...
void	ver_intersection_calcul(void)
{
	rays_directions();
	normalise_angle();
	g_foundawallver = 0;
	...
	g_yinteruptver = g_new_player.pos_y + ((g_xinteruptver - g_new_player.pos_x)
	* tan(g_new_player.ray_angle));
	g_xstepver = g_tile;
